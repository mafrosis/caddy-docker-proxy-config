...
#
FROM caddy:${CADDY_DOCKER_TAG}-builder AS builder-ts
...
FROM caddy:${CADDY_DOCKER_TAG}-alpine AS caddy-ts
...
COPY --from=builder-ts /usr/bin/caddy /usr/bin/caddy
...
FROM caddy:${CADDY_DOCKER_TAG}-alpine AS caddy
